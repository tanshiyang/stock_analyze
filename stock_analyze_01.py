...
def every_date(start_date, end_date, table_name):
    # 获取所有有股票
        ...
        try:
            ...
            if re.match('000', codes[x]) or re.match('002', codes[x]):
                ...
                while times <= 5 and sql == "":
                    times = times + 1
                    sql = one_stock(codes[x], start_date, end_date, times, table_name)
...
def one_stock(code, start_date, end_date, times, table_name):
    if times > 5:
        return ""
    try:
        ts_code = code + ".SZ"
        ...
        fina_indicator = pro.fina_indicator(ts_code=ts_code, start_date=start_date, end_date=end_date)
...
every_date('20181001', '20190101','stock_analyze_1804')
