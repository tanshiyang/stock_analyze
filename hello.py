...
def every_date(starttime, endtime):
    # 获取所有有股票
    ...
    cursor.execute('create table stock_analyze_01 (ts_code varchar(32),close1 float, close2 float,total_revenue float, adv_receipts	float, '
                   'unique(ts_code))')
    ...
    for x in range(0, len(stock_info)):
        ...
        if re.match('000', codes[x]) or re.match('002', codes[x]):
            ...
            cursor.execute('create table stock_' + codes[x] + '(date varchar(32),open varchar(32),close varchar(32),'
                                                              'high varchar(32),low varchar(32),volume varchar(32),'
                                                              'p_change varchar(32),unique(date))')
...
every_date('2018-01-01', '2018-03-14')
